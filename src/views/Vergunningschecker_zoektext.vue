  ...
  <div>
    ...
    <b-row v-if="step==1">
      ...
      <div class="formlines">Voer het adres in waar u de uitbouw wilt gaan plaatsen</div>
        ...
        <!-- <div v-for="item in zoekresultaten" v-bind:key="item.identificatie">{{item.omschrijving}}</div> -->
...
<!-- <b-dropdown id="dropdown-1" text="Dropdown Button" class="m-md-2">
    <b-dropdown-item v-for="item in zoekresultaten" v-bind:key="item.identificatie">{{item.omschrijving}}</b-dropdown-item>
</b-dropdown> -->

<!-- https://dev.to/vuesomedev/autocomplete-dropdown-using-native-html-moi -->

<!-- <input v-model="zoektext" list="zoekresultaten" placeholder="Start typing..." @keypress="zoekAdres($event)"> -->

<div class="entryform">
  <input v-model="zoektext" list="zoekresultaten" placeholder="Start typing..." v-on:keyup.enter="submit" >
  <datalist id="zoekresultaten" >
    <option v-for="item in zoekresultaten" v-bind:key="item.identificatie" >{{item.omschrijving}}</option>  
  </datalist>

<!-- <u id="list">
  <li v-for="item in zoekresultaten" v-bind:key="item.identificatie" >
    {{item.omschrijving}}
  </li>
</u> -->
...
export default {
  ...
  computed:{
    ...
    found_address: function(){
        ...
        return this.zoekresultaten.length == 1;
    },
  ...
  watch: {
    ...
    zoekresultaten: function(){
          ...
          for(var i= 0; i < this.zoekresultaten.length ;  i++){
            if(this.zoekresultaten[i].omschrijving == this.zoektext){

              var adres = this.zoekresultaten[i];

              this.zoekresultaten = [];
              this.zoekresultaten.push(adres);

              this.zoekAdres2(adres.identificatie);
              console.log("vette shit man " + this.zoekresultaten[i].identificatie);
              break;
            }
          ...
          //   console.log("we have one result! " + this.zoekresultaten[0].identificatie);
          //   //console.log("we have one result! " + this.zoekresultaten[0]._links.adres.href);
          //   this.zoekAdres2(this.zoekresultaten[0].identificatie);
          // }
    ...
    zoekAdres:function(text){
      ...
      fetch(url, { headers })
        ...
        .then(data => {
          ...
          console.log(data._embedded.zoekresultaten);
        });

    },
    zoekAdres2:function(id){
...
//          console.log(data);
    ...
    getAddress: function(postcode, huisnummer){
      ...
      let url = `https://api.bag.kadaster.nl/lvbag/individuelebevragingen/v2/adressen/zoek?zoek=${postcode} ${num}&page=1&pageSize=20`;

      fetch(url, { headers })
        .then(response => response.json())
        .then(data => {
            //  this.zoekresultaten = data._embedded.zoekresultaten.replace("\"", "");
          console.log(data._embedded.zoekresultaten);
